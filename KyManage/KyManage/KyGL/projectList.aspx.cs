...
namespace KyManage.KyGL
{
    public partial class projectList : System.Web.UI.Page
    {
       ...
       private void dataBind()
        {
            string sql = "";
            sql += "select a.*,b.name as teacher_name from projectinfo as a left join teacherinfo as b on a.teacher_number=b.teacher_id where 1=1";
            if (TextName.Text.Trim() != "")
            {
                sql += " and a.project_number like '%" + TextName.Text.Trim() + "%'";
            }
            sql += " order by a.prjid desc";
        ...
        protected void btnDelAll_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                Class.DelInfo("delete from projectinfo where prjid in (" + ggid + " )");
                WebJS.Alert("删除完成");
                dataBind();
        ...
        protected void infoList_RowDeleting(object sender, GridViewDeleteEventArgs e)
        {
            ...
            try
            {
                data.Exesql("delete from projectinfo where prjid=" + id);
                dataBind();
            }
            catch (Exception ex)
            {
