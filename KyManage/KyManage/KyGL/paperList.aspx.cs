...
namespace KyManage.KyGL
{
    public partial class paperList : System.Web.UI.Page
    {
        ...
        {
            string sql = "";
            sql += "select a.*,b.name as teacher_name from paperinfo as a left join teacherinfo as b on a.teacher_number=b.teacher_id where 1=1";
            if (TextName.Text.Trim() != "")
            {
                sql += " and a.paper_number like '%" + TextName.Text.Trim() + "%'";
            }
        ...
        protected void btnDelAll_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                Class.DelInfo("delete from paperinfo where id in (" + ggid + " )");
                WebJS.Alert("删除完成");
                dataBind();
        ...
        protected void infoList_RowDeleting(object sender, GridViewDeleteEventArgs e)
        {
            ...
            try
            {
                data.Exesql("delete from paperinfo where id=" + id);
                dataBind();
            }
            catch (Exception ex)
            {
